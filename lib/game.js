...
class Game {
  ...
  drawEnemy() {
    var create = setInterval(() => this.enemies.push(new Enemy(this.ctx, this.tankImg)), 333)
    if (this.enemies.length >= this.stage) {
      console.log("hit", this.enemies.length, this.stage);
  ...
  gameover() {
        ...
        this.ctx.font="20px Lobster";
        this.ctx.fillText(`Press R to restart the game`,120,300);
  }
